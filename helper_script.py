...
class Youtube_Helper:
    ...
    def save_event_video(video_id, time, yolo_overlay=False, folder="event_videos"):
        ...
        try:
            ...
            video.close()
            Youtube_Helper.trim_video(input_path, output_path, start_time, end_time)
            logger.debug(f"Saved video of event to {output_path}.")
            if yolo_overlay:
                output_path_overlay = os.path.join(folder, f"{video_id}_{time}.mp4")
                Youtube_Helper.tracking_mode(output_path, output_path_overlay, video_fps=video_fps)
                logger.debug(f"Saved video of event with YOLO overlay to {output_path}.")
        except Exception as e:
